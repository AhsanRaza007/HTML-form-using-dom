let returnFood = (name)=>{
    ...
    food.className = 'form-group form-check col-md-1 col-6 ';
...
root.className = 'container my-5 mx-auto jumbotron';
...
root.appendChild(form);
    ...
    firstName.className = 'col-md-5 form-control my-1 firstName';

    let lastName = document.createElement('input');
    lastName.setAttribute('placeholder', 'Last Name');
    lastName.className = 'col-md-5 form-control my-1 lastName';

    let address = document.createElement('input');
    address.setAttribute('placeholder', 'Address');
    address.className = 'col-md-5 form-control my-1 address';

    let pincode = document.createElement('input');
    pincode.setAttribute('placeholder', 'Pincode');
    pincode.className = 'col-md-5 form-control my-1 pincode';

    let foods = document.createElement('div');
    foods.className = 'row col-12 mt-3 justify-content-around';
        let apple = returnFood('apple');
        let chicken = returnFood('chicken');
        let spinach = returnFood('spinach');
        let carrot = returnFood('carrot');
        let tomato = returnFood('tomato');
    foods.append(apple, chicken, spinach, carrot, tomato);
    
    let genderDiv = document.createElement('div');
    genderDiv.className = 'col-12 row my-2 justify-content-center';
    ...
    genderDiv.append(maleRadio,maleLabel,femaleRadio, femaleLabel);

    let state = document.createElement('input');
    state.setAttribute('placeholder', 'State');
    state.className = 'col-md-5 form-control my-1 state';

    let country = document.createElement('input');
    country.setAttribute('placeholder', 'Country');
    country.className = 'col-md-5 form-control my-1 country';

    
    let submitButton = document.createElement('button');
    submitButton.className = 'btn btn-primary py-2 px-5 btn-lg mt-5 col-md-5';
    submitButton.setAttribute('type', 'submit');
    submitButton.innerHTML = 'submit';

form.append(firstName, lastName, address, pincode, foods,genderDiv, state, country, submitButton);
...
root.appendChild(table);
    ...
    let headingRow = document.createElement('tr');
        ...
        let pincodeHeading = document.createElement('th');
        pincodeHeading.innerHTML = 'Pincode';

        let foodHeading = document.createElement('th');
        foodHeading.innerHTML = 'Foods';
        
        let genderHeading = document.createElement('th');
        genderHeading.innerHTML = 'Gender';

        let stateHeading = document.createElement('th');
        stateHeading.innerHTML = 'State';

        let countryHeading = document.createElement('th');
        countryHeading.innerHTML = 'Country';

    headingRow.append(firstNameHeading, lastNameHeading, addressHeading, pincodeHeading, foodHeading, genderHeading, stateHeading, countryHeading);
